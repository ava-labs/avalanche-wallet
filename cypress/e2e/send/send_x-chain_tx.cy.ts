...
describe('Send transaction with x-chain balance', () => {
    beforeEach(() => {
        ...
        cy.get('div.new_order_Form > div.lists > div:nth-child(1) > div > button:nth-child(1)')
            .as('btnSourceX')

        cy.get('div.header > button:nth-child(3)')
            .as('btnBreakdown')

        cy.get('div.alt_info > div > div:nth-child(1) > p:nth-child(2)')
            .as('pXAvailBalance')

        cy.get('div.max_in_cont.hover_border > div > input')
            .as('inputAmount')

        cy.get('div > div.col_balance > p')
            .as('pMaxBalance')

        cy.get('div.bottom_tabs > div > button:nth-child(1)')
            .as('btnXChain')

        cy.get('div.bottom > div.bottom_rest > p.addr_text')
            .as('pAddress')

        cy.get('[data-cy="wallet_address"]')
            .as('cyWalletAddress')

        cy.get('div.new_order_Form > div:nth-child(2) > div.to_address > div > input')
            .as('inputToAddress')

        cy.get('div.new_order_Form > div:nth-child(2) > div.checkout > button')
            .as('btnConfirm')

        cy.get('div.checkout > button.button_primary.v-btn.v-btn--block.v-btn--depressed.theme--light.v-size--default')
            .as('btnSend')
    ...
    it('Send transaction with all available x-chain balance', () => {
        ...
        cy.get('@pXAvailBalance').invoke('text').then((text) => {
            ...
            cy.get('@pMaxBalance').click().invoke('text').then((text) => {
            ...
            cy.get('@inputAmount').invoke('val').then((val) => {
        ...
        cy.get('@cyWalletAddress').invoke('text').then((text) => {
        ...
        cy.wait('@xEndpoints').then((intercept) => {
            if (intercept.request.body.method === 'avm.issueTx') {
                ...
                // expect display txID 
                cy.get('div.new_order_Form > div:nth-child(2) > div.checkout > label').invoke('text').then((text) => {
                ...
                cy.get('@currentRpcHost').then(currentRpcHost => {
                    cy.request({
                        ...
                        url: currentRpcHost.protocol + '//' + currentRpcHost.host + ':' + currentRpcHost.port + '/ext/bc/X',
                        body: {
                            "jsonrpc": "2.0",
                            "id": 1,
                            "method": "avm.getTx",

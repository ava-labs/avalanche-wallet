...
xChain
    .importKey(geckoUsername, geckoPass, faucetKey.getPrivateKeyString())
    .then(async (res) => {
...
async function sendAvax(address: string, amount: number) {
    ...
    let unsigned_tx = await xChain.buildBaseTx(
        utxoSet,
        sendAmount,
        assetId,
        [address],
...
async function createFixedCapAsset(
    name: string,
    symbol: string,
    address: string,
    amount: number,
    denomination: number = 2
) {
    let holders = [
        {
            address: address,
    ...
    await xChain.createFixedCapAsset(
        geckoUsername,
        geckoPass,
        name,
        symbol,
